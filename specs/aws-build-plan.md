...
- hosted app defaults to cloud mode on the Amplify URL
- local preview defaults to local prototype mode
- app has cloud/local mode switching
- cloud sign-in uses the deployed Cognito user pool client
- cloud signup uses the control-plane email verification flow
- cloud band list/create uses the Bandmanager API
- cloud join links are generated by the Bandmanager API and redeem with `?join=...`
- cloud calendar event create/edit/cancel calls the Bandmanager API
- local mode still uses browser storage for prototype setlists, posters, and mock data

Frontend smoke test:

- local mode rendered with no console errors
- cloud auth screen rendered with no console errors
- temporary Cognito user signed in from the frontend
- cloud band creation from the frontend succeeded
- temporary test user and test band records were cleaned up
